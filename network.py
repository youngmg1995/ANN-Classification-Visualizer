...
class Network(object):
    ...
    def evaluate(self, data_set):
        """Evaluates all examples in the passed dataset using feedforward
        Returns results for each example as array."""
        results = []
        for example in data_set:
            results.append(np.argmax(self.feedforward(example[0])))
        return results
